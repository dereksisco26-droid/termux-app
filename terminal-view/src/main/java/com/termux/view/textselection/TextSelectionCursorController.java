...
public class TextSelectionCursorController implements CursorController {
    ...
    public TextSelectionCursorController(TerminalView terminalView) {
        ...
        mStartHandle = new TextSelectionHandleView(
            terminalView,
            this,
            TextSelectionHandleView.LEFT
        );
        mEndHandle = new TextSelectionHandleView(
            terminalView,
            this,
        ...
        mHandleHeight = Math.max(
            mStartHandle.getHandleHeight(),
            mEndHandle.getHandleHeight()
        );
    }
    ...
    public void show(MotionEvent event) {
        ...
        mStartHandle.positionAtCursor(
            mSelX1,
            mSelY1,
            true
        );

        mEndHandle.positionAtCursor(
            mSelX2 + 1,
            mSelY2,
            true
        );
                ...
                switch (item.getItemId()) {
                    ...
                    case ACTION_COPY:
                        ...
                        terminalView.mTermSession
                            .onCopyTextToClipboard(selectedText);
                    ...
                    case ACTION_SELECT_ALL:
                         ...
                         *
                         * screen.getActiveRows() contains the number of rows
                         * currently available in the terminal buffer.
                         *
                         * The visible terminal rows occupy:
                         *
                         *     0 ... mRows - 1
                         *
                         * Scrollback extends upward into negative Y coordinates.
                         */
                        TerminalBuffer screen =
                            terminalView.mEmulator.getScreen();

                        int scrollRows =
                            screen.getActiveRows() -
                            terminalView.mEmulator.mRows;

                        mSelX1 = 0;
                        mSelY1 = -scrollRows;

                        mSelX2 =
                            terminalView.mEmulator.mColumns - 1;

                        mSelY2 =
                            terminalView.mEmulator.mRows - 1;

                        /*
                         * Move the viewport to the bottom of the terminal
                         * so that the complete selection is represented
                         * correctly by the selection handles.
                         */
                        ...
                        /*
                         * Do NOT stop text selection mode here.
                         *
                         * This allows the user to select:
                         *
                         * Select all -> Copy
                         *
                         * without reopening the selection menu.
                         */
                        break;

                    case ACTION_MORE:
                        // We first store the selected text in case
                        // TerminalViewClient needs the selected text before
                        // MORE button was pressed since we are going to
                        // stop selection mode.
                        mStoredSelectedText = getSelectedText();

                        // The text selection needs to be stopped before
                        // showing context menu, otherwise handles will show
                        // above popup.
                        terminalView.stopTextSelectionMode();
        ...
        mActionMode = terminalView.startActionMode(
            new ActionMode.Callback2() {
                ...
                public boolean onCreateActionMode(
                    ...
                    return callback.onCreateActionMode(
                        mode,
                        menu
                    );
                }
                ...
                public boolean onActionItemClicked(
                    ...
                    return callback.onActionItemClicked(
                        mode,
                        item
                    );
                }
                ...
                public void onGetContentRect(
                    ...
                    int x1 = Math.round(
                        mSelX1 *
                        terminalView.mRenderer.getFontWidth()
                    );
    ...
    public void updatePosition(
        TextSelectionHandleView handle,
        int x,
        int y
    ) {
        TerminalBuffer screen =
        ...
        if (handle == mStartHandle) {
            ...
            if (
                !terminalView.mEmulator.isAlternateBufferActive()
            ) {
                ...
                if (mSelY1 <= topRow) {
                    ...
                    mSelY1 >=
                    topRow + terminalView.mEmulator.mRows
                ) {
                    topRow++;
